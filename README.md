...
Прямые версии закреплены в `requirements*.txt`; версии фактически установленных основных библиотек записываются в manifest. Сборка скачивает Python-образ и пакеты; готовый анализ работает без внешней сети.
...
Для другого расположения файлов задайте `APEXFLOW_DATA_DIR` и `APEXFLOW_OUTPUT_DIR`; значения по умолчанию — `./data` и `./output`. Повторный расчёт обновляет сгенерированные CSV и manifest в выбранном output. Чтобы сохранить прошлый расчёт, создайте другую папку и задайте её через `APEXFLOW_OUTPUT_DIR` перед запуском.
