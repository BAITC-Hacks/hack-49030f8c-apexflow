...
| Высокая центральность | `Q90(betweenness)`; если положительных значений нет, coordinator отключён | ≈0,000388278615 |

Числа проверены в закреплённом Docker runtime с NetworkX 3.4.2. Пороги пересчитываются по данным; значения раннего эксперимента с другой версией NetworkX сохранены отдельно в методологии и не подменяют эти измерения.
...
Повторно подтверждённые результаты проверки staging `d7c180c` от 23 сентября 2026 года. Исполняемый код совпадает с ранее проверенным `91ead09`; ниже приведены результаты нового запуска:
...
| Полный реальный расчёт | 0,617 и 0,596 с на Apple M2, 8 CPU, 16 GiB RAM; Docker VM — около 7,75 GiB RAM |
