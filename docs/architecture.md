...
Pipeline и UI используют общую Docker runtime-базу Python 3.12 с закреплёнными прямыми зависимостями; tests — отдельная цель той же базы и Compose-профиль. Образ не содержит данных, результатов или секретов. Runtime работает без root, UI опубликован только на `127.0.0.1:8501`. Compose ожидает успешного pipeline перед стартом интерфейса.
...
Каждый CSV заменяется атомарно. Manifest связывает набор: UI отвергает незавершённый/неуспешный запуск или несовпадающие хеши. Проверка выполняется до и после чтения; изменение сигнатур прерывает чтение. Перед использованием кэша UI снова проверяет manifest и хеши. Неподвижная страница не опрашивает диск непрерывно, данные проверяются при следующем действии пользователя. Рекомендованный пересчёт — с остановленным UI.
