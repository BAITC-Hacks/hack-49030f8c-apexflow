...
def _render_node_card(data: ViewData, node: pd.Series) -> None:
    ...
    if bool(node["is_seed"]):
        limitations.append("Для стартового узла входящие операции извне наблюдаемого графа могут быть неполными даже за тот же период.")
        steps.append("Запросить полные входящие операции за наблюдаемый период, включая плательщиков вне выгруженного графа.")
